...
from models.concorrencia_distribuida_Claude import *
...
servidor_RA = RicartAgrawalaServer(id_server, SERVERS)
servidor_RA.start()
...
def tratar_conexao(conexao, usuarios, passagens, rotas):
    try:
        while True:
            ...
            # Solicita acesso à seção crítica antes de acessar clientes.json
            servidor_RA.request_critical_section()
            
            try:
                # Recarrega usuários já que estamos na seção crítica
                ...
                if opcode == 1:
                    ...
                    resultado = comprar_passagem(conteudo['cliente_id'], 
                                              conteudo['rotas_a_serem_compradas'],
                                              rotas, passagens, usuarios)
                elif opcode == 4:
                    resultado = buscar_passagens_de_usuario(conteudo['cliente_id'],
                                                          usuarios)
                elif opcode == 5:
                    resultado = cancelar_passagem(conteudo['id_passagem'],
                                               conteudo['userID'],
                                               passagens, usuarios)
                else:
                ...
                # Libera a seção crítica após o processamento
                servidor_RA.release_critical_section()
